...
const SINGLE_TEMPLATE_ARV_HARDENING =
  'Keep it tactile, adult, calm-trippy, and immediately readable: one dominant subject or event, centered or clearly intentional composition, sparse composition, clean lower third for OBS overlays, locked or very slow deliberate camera, strong material logic, coherent contrast, a scene-led palette, and loop-capable motion. Bias toward dark, nocturnal, or void-held environments unless the concept strongly needs another setting. Let the world choose its own surfaces instead of defaulting to archive-witness, paper-cut, monolith, or stone-relic imagery. Avoid childish softness, mascot design, glossy fantasy-cartoon finish, daylight realism, or crowd clutter.';
...
function buildStoryTemplatePrompt(
  ...
  return `${softenedBasePrompt} Build it as an exact 4-scene video arc coded as scene 1 emergence, scene 2 lock-in, scene 3 peak, scene 4 afterimage, with escalating reveals, tactile motion in every scene, and at least one strange transformation or environmental reaction. Treat every scene as a clear loop-ready moment with one dominant subject, one decisive motion payoff, and a final image that either hands off cleanly to the next scene or hangs as a repeatable afterimage. Keep continuity, but let the world become bolder, odder, more cinematic, and more instantly readable from scene to scene. Keep the style adult, sparse, calm-trippy, and tactile: locked or very slow camera, centered or clearly intentional framing, clean lower third for OBS overlays, strong material response, coherent contrast, a scene-specific palette, visible loop logic, and a preference for dark, nocturnal, or void-biased space unless the prompt clearly needs another setting. Do not default back into archive-witness, paper-cut, monolith, or stone-relic canon unless the prompt explicitly asks for it. Explicitly avoid cute, mascot-like, whimsical, glossy, fantasy-cartoon language, or random daylight realism. If text, labels, or branding appear, keep them subtle, secondary, and non-dominant.${direction}`;
}
...
const STILLFRAME_PROMPT_TEMPLATES: PromptTemplate[] = [
  ...
  },
  {
    ...
    target: 'both',
    suggestedPresetId: 'stillframe-rituals',
    prompt: 'A strange saint-like silhouette made from torn circuit paper stands in a dark analog chapel. A thin glowing line across the chest pulses once like a quiet flatline signal, then fades back into stillness. The figure never moves. Sacred geometry faintly visible behind the head, film grain, dust, paper fibers, cyan glow, rust amber shadows, black velvet negative space. Micro-motion stop-frame rhythm: move, pause, hold, return. Seamless loop, 4 to 6 seconds, no text, no watermark.',
  },
  {
    id: 'sr-dust-oracle',
    label: 'Dust Oracle',
  ...
  },
  {
    ...
    target: 'both',
    suggestedPresetId: 'stillframe-rituals',
    prompt: 'A pale cardboard moon hangs above a motionless silhouette. A tiny orbit diagram around the moon rotates slightly, glitches once, then settles back into place. The scene holds like a forgotten scientific poster. Black velvet, faded cream, cobalt blue, rust amber, cyan glow, soft film grain, paper fibers, scanlines, dusty projector light. Locked wide shot, one micro-event only, seamless 4–6 second GIF-style loop, lower third clean for OBS. No subtitles, no logo, no aggressive flashing.',
  },
  {
    id: 'sr-candle-signal',
    label: 'Candle Signal',
